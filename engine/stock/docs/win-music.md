...
8. Select track 4 and place the first lead motif with the inherited 96-tick
   ...
   `dur 48`. Hold Ctrl and marquee across all four notes. Its time edge follows
   the pointer exactly while its top and bottom expand to complete pitch rows;
   each intersecting note highlights before you release. Press **Ctrl+C**. The
   four-note motif is now on the session clipboard.
