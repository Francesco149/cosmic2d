...
function game.init()
  ...
  cam = cb
  if cam:f32(0) == 0 and cam:f32(8) == 0 then
...
local function angdiff(target, from)
  return m.atan2(m.sin(target - from), m.cos(target - from))
end

-- orbit-follow camera: the godot FollowCamera rig (openworld copy)
local function cam_step()
  ...
  if rec == 0 and hold <= 0 and kc.yaw_follow > 0 and hspeed > kc.min_speed then
    ...
    if m.abs(da) < m.pi - kc.back_cone then -- D3D-018 back cone
