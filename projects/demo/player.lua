...
function M.init()
  ...
  local ok, b = pcall(pal.buf, "demo.player", SIZE)
  if not ok then
    pal.buf_free("demo.player")
    b = pal.buf("demo.player", SIZE)
  end
  buf = b
...
-- state accessors for the game layer (sfx hooks, room logic)
function M.grounded() return buf:f32(O.grounded) == 1.0 end
function M.vy() return buf:f32(O.vy) end
function M.size() return M.W, M.H end

-- GREYBOX: hard-place the player (portal arrival / map switch); zero momentum
function M.warp(x, y)
  buf:f32(O.x, x); buf:f32(O.y, y)
  buf:f32(O.vx, 0); buf:f32(O.vy, 0)
end
