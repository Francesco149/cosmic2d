...
| `pal.x_path_info(p)` / `pal.x_file_publish(temp,dest)` / `pal.x_windows_exe_identity(...)` | dev/io | API v14: path type/size plus link detection; publication syncs a complete sibling temp and renames it to a previously absent artifact name, leaving the temp non-authoritative on failure. Windows can brand a private GUI-engine copy with validated project icon/version resources before it enters the ZIP; Linux reports that target-specific seam unavailable |
