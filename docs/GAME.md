...
| 6 | **Flutter** | hold E after a hop, **into the fall** | **rhythmic hover** (*M7 feel pass 4*, 2026-06-28 — replaces the old slow-fall glide): once you've started FALLING (past the hop's apex) and you're still holding, a **mini-hop every `flutter_interval`** (a height-based up-kick `flutter_h` sized to **roughly HOLD your altitude** over the beat + a small forward nudge `flutter_vx`), **`flutter_boosts` times** (≈ a few s), then **`hop_cd`**. **A plain hop never triggers the cooldown** — released before you fall, it's a clean hop (the "must be falling" gate *is* the tap guard). `flutter_h` ↑ = more hold / bigger bob, ↓ = a gentle descent | `flutter_interval`(60f), `flutter_boosts`(4), `flutter_h`(45), `flutter_vx`(70), `hop_cd`(600f) | cooldown persists across airtimes / boost puffs + hover shimmer |
