...
| 2 | **Jump** | Jump, grounded | fixed apex **≈1 CH** (not variable-height); **hold Jump auto-repeats** on landing | `jump_h`, `jump_apex_t` | / takeoff puff |
...
| 4 | **Up jump** | hold Up + Jump, airborne | vertical boost (old double-jump, but up). **Fixed impulse** — no extra height from holding; time it for less height. Perfect chain ≈ 5 CH total | `upjump_v` | **once per airtime**; **locks out flash-jump until landing**; no up-jump→flash-jump / vertical burst |
...
| 6 | **Flutter** | hold E after a hop | hover / slow-fall up to **10 s**. Then **hop goes on a 10 s cooldown** — full 10 s elapsed *or* the moment you release. **A plain hop never triggers the cooldown**: the hover (and its cd) only *engages* after E is held past `flutter_grace` airborne frames (*M7 feel pass 2* — a normal tap, not just a 1-frame one, is a clean hop) | `flutter_grace`(10f), `flutter_max`(600f), `flutter_fall`, `hop_cd`(600f) | cooldown persists across airtimes / hover shimmer |
| 7 | **Grapple** | `q` (the spec's \` is the dev console) | the hook **EXTENDS** to a standable top above at ~1 screenful/s **while you stay under gravity**, then **reels** you in from the velocity you connect with (*M7 feel pass 2*: a jump-into-grapple has fallen back to downward velocity by then, which damps the reel — no short-range slingshot). Max range ≈ 1 screenful; **prefers a target beyond ½ a screenful** (reaches past close ones). Jump **cancels** either phase (does *not* auto-FJ). Chains anywhere | `grapple_range_max`, `grapple_range_min_pref`, `grapple_extend`, `grapple_accel`, `grapple_vmax`, `grapple_cd`(180f) | **once per airtime**; **3 s cooldown** after finish/cancel / beam + pull |
